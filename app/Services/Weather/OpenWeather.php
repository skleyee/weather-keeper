...
class OpenWeather implements WeatherService
{
    ...
    {
        try {
            $response =$this->httpClient->get(config('weather.api_url') . '/data/2.5/weather?lat=' . $coordinates['lat'] . '&lon=' . $coordinates['lon'] . '&appid=' . config('weather.api_key'));
    ...
    {
        try {
            $response =$this->httpClient->get(config('weather.api_url') . '/geo/1.0/direct?q=' . $city . '&limit=1&appid=' . config('weather.api_key'));
